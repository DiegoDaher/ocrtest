...
def keyword_match(line: str, keyword: str) -> re.Match[str] | None:
    return keyword_pattern(keyword).search(line)


def line_contains_keywords(line: str, keywords: tuple[str, ...]) -> bool:
    return any(keyword_match(line, keyword) for keyword in keywords)
    ...
    *,
    ...
    for idx, line in enumerate(lines_list):
        for keyword in keywords:
            match = keyword_match(line, keyword)
            if not match:
                continue
            ...
            for offset in range(1, max_lookahead + 1):
                ...
                if not next_value:
                    continue
                if stop_keywords and line_contains_keywords(next_value, stop_keywords):
                    break
...
def parse_compact_birth_date(compact: str) -> str | None:
...
def compact_birth_candidates(token: str) -> list[str]:
    if len(token) == 8:
        return [token]
    if len(token) != 9:
        return []
...
def extract_compact_birth_date(value: str) -> str | None:
    for token in re.findall(r"\d{8,9}", value):
        for candidate in compact_birth_candidates(token):
            parsed = parse_compact_birth_date(candidate)
            if parsed:
                return parsed
    return None


def normalize_curp_candidate(candidate: str) -> str | None:
    if len(candidate) != 18:
        return None
...
def extract_curp(normalized_text: str) -> str | None:
    ...
    for chunk in compact_chunks:
        ...
        for start in range(max_start + 1):
            ...
            normalized_candidate = normalize_curp_candidate(candidate)
            if normalized_candidate:
                return normalized_candidate
    return None


def extract_birth_date(lines: list[str], normalized_text: str) -> str | None:
    birth_keywords = ("NACIMIENTO", "NAC1MIENTO", "NACIMENTO", "NAC1MENTO")
    for idx, line in enumerate(lines):
        if line_contains_keywords(line, birth_keywords):
            match = DATE_REGEX.search(line)
            if match:
                return match.group(1)
            compact_match = extract_compact_birth_date(line)
            if compact_match:
            ...
            if idx + 1 < len(lines):
                ...
                next_compact_match = extract_compact_birth_date(next_line)
...
def extract_validity(lines: list[str]) -> str | None:
    validity_keywords = ("VIGENCIA", "VENCE")
    for idx, line in enumerate(lines):
        if line_contains_keywords(line, validity_keywords):
...
def extract_name_tokens(value: str) -> list[str]:
    ...
    if keyword_match(cleaned, "DOMICILIO") or ADDRESS_HINT_REGEX.search(cleaned):
        return []
...
def extract_ine_name_block(lines: list[str], max_lookahead: int = 8) -> str | None:
    name_keywords = ("NOMBRE", "NOMBRES")
    for idx, line in enumerate(lines):
        if not line_contains_keywords(line, name_keywords):
            continue
        ...
        for keyword in name_keywords:
            match = keyword_match(line, keyword)
            if match:
                fragments.extend(extract_name_tokens(line[match.end() :]))
                break
        ...
        for offset in range(1, max_lookahead + 1):
            ...
            if line_contains_keywords(next_line, NAME_STOP_KEYWORDS):
                break
            if fragments and ":" in next_line:
                break

            tokens = extract_name_tokens(next_line)
            if not tokens:
        ...
        if looks_like_person_name(compact_name):
            return compact_name
    return None


def mrz_compact(line: str) -> str:
    return re.sub(r"[^A-Z0-9<]", "", line)


def parse_mrz_date(value: str, is_validity: bool) -> str | None:
...
def extract_from_mrz(lines: list[str]) -> tuple[str | None, str | None, str | None]:
    mrz_name = None
    mrz_birth_date = None
    mrz_validity = None
        ...
        if not mrz_birth_date or not mrz_validity:
            ...
            if date_match:
                if not mrz_birth_date:
                    mrz_birth_date = parse_mrz_date(date_match.group("birth"), is_validity=False)
                if not mrz_validity:
                    mrz_validity = parse_mrz_date(date_match.group("valid"), is_validity=True)

    return mrz_name, mrz_birth_date, mrz_validity


def clean_line_for_address(line: str) -> str:
    cleaned = re.sub(r"[^A-Z0-9,.\- ]", " ", line)
    return re.sub(r"\s+", " ", cleaned).strip(" ,.-")


def extract_address_fallback(lines: list[str]) -> str | None:
    for idx, line in enumerate(lines):
...
def extract_curp_certification(lines: list[str]) -> tuple[str | None, bool | None]:
    if not lines:
        return None, None

    full_text = " ".join(lines)
    has_certified = line_contains_keywords(full_text, ("CURP CERTIFICADA", "CERTIFICADA"))
    has_registry = line_contains_keywords(full_text, ("REGISTRO CIVIL",))
    has_verified = line_contains_keywords(full_text, ("VERIFICADA", "VERIFICADO"))
